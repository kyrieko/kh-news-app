...
// POST /api/crawl
// 헤더: x-crawl-secret — 환경 변수 CRAWL_SECRET_KEY 와 비교해 인증
export async function POST(request: NextRequest) {
  // 1. secret key 인증
  const secret = request.headers.get('x-crawl-secret');
  if (!process.env.CRAWL_SECRET_KEY || secret !== process.env.CRAWL_SECRET_KEY) {
    return NextResponse.json({ error: '인증 실패' }, { status: 401 });
  }

  // 2. DB에서 카테고리 전체 조회 → slug → id 매핑 테이블 생성
  ...
  // 4. 순차 처리 (Claude API rate limit 고려)
